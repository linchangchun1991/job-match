...
import { Database, Zap, Lock, AlertTriangle, CheckCircle, Trash2, Upload } from './Icons';
...
const JobManager: React.FC<JobManagerProps> = ({ jobs, onUpdate, onRefresh, readOnly = false, defaultOpen = false }) => {
  ...
  const processUpload = async (shouldClear: boolean) => {
    ...
    try {
        ...
        if (rawJobs.length === 0) throw new Error("识别失败。请检查格式：公司 | 岗位 | 地点");
        ...
        if (result.success) {
          setSyncStatus({ type: 'success', msg: `同步成功！已更新 ${formattedJobs.length} 条岗位` });
          setPasteContent('');
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500">当前库存：{jobs.length} 个岗位 | 系统已自动连接云端</p>
          </div>
        </div>
      </div>
      
      <div className="bg-[#0a0a0f] border border-[#1c1c21] rounded-2xl p-8 shadow-2xl">
        <div className="space-y-6">
          <textarea
            className="w-full h-64 bg-black/60 border border-white/5 rounded-2xl p-6 text-xs font-mono text-blue-100/70 focus:border-blue-500/50 focus:outline-none transition-all placeholder-gray-700 custom-scrollbar leading-relaxed"
          ...
          {syncStatus.msg && (
             <div className={`p-4 rounded-xl flex items-center gap-3 animate-in slide-in-from-top-2 ${syncStatus.type === 'success' ? 'bg-green-500/10 border border-green-500/20 text-green-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>
